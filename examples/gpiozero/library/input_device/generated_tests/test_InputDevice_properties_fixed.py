...
@given(st.booleans())
def test_active_state_affects_is_active_basic(pull_up):
    Device.pin_factory = MockFactory(pin_class=MockPin)
    with InputDevice(4, pull_up=pull_up) as device:
...
#     Device.pin_factory = MockFactory(pin_class=MockPin)
#     with InputDevice(4, pull_up=pull_up, active_state=pin_state) as device:
#         if pull_up:
#             assert device.is_active == (not pin_state)  # Pull-up inverts
#         else:
#             assert device.is_active == pin_state
...
@given(st.booleans(), st.integers(min_value=4, max_value=10))

...
def test_initialization_leds_off(delay, number):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with patch('examples.gpiozero.apps.app_6.src.presence_indicator.requests') as mock_requests:
...
@given(st.floats(min_value=0.1, max_value=5.0), st.integers(min_value=1, max_value=10))
@settings(max_examples=10)
def test_leds_value_proportional_to_number(delay, number_returned):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with patch('examples.gpiozero.apps.app_6.src.presence_indicator.requests') as mock_requests:
        ...
        with PresenceIndicator(url="http://example.com", delay=delay) as pi:
            ...
            expected_value = min(number_returned / 10.0,  1.0)
...
@settings(max_examples=10)
@given(st.floats(min_value=0.1, max_value=5.0), st.integers(min_value=1, max_value=99999))
def test_stop_playing_sets_playing_to_false(delay, number):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with patch('examples.gpiozero.apps.app_6.src.presence_indicator.requests') as mock_requests:
        mock_requests.get.return_value.json.return_value = {'number': number}
...
@given(st.floats(min_value=0.1, max_value=5.0), st.integers(min_value=1, max_value=100))
@settings(deadline=1000, max_examples=10)
def test_exit_turns_leds_off(delay, number_returned):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with patch('examples.gpiozero.apps.app_6.src.presence_indicator.requests') as mock_requests:
        mock_requests.get.return_value.json.return_value = {'number': number_returned}

        with PresenceIndicator(url="http://example.com", delay=delay) as pi:
            sleep(0.2)  # Run play() briefly

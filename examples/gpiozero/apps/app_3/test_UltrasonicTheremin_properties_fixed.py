...
def test_distance_to_tone_in_range(distance_value):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with UltrasonicTheremin(27, 17, 21, octaves=3) as mock_theremin:
...
def test_distance_to_tone_near_min(distance_value):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with UltrasonicTheremin(27, 17, 21, octaves=3) as mock_theremin:
        tone = mock_theremin.distance_to_tone(distance_value)
        assert abs(
...
def test_distance_to_tone_near_max(distance_value):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with UltrasonicTheremin(27, 17, 21, octaves=3) as mock_theremin:
        tone = mock_theremin.distance_to_tone(distance_value)
...
@given(octaves=st.integers(min_value=1, max_value=4))
def test_octaves_affects_tone_range(octaves):
    ...
    with UltrasonicTheremin(27, 17, 21, octaves=octaves) as ut:
        ...
        assert (ut.buzzer.max_tone.midi - ut.buzzer.min_tone.midi) == expected_range * 2;


@given(distance_value=st.floats(min_value=0.0, max_value=1.0), octaves=st.integers(min_value=2, max_value=3))
def test_distance_tone_monotonic(distance_value, octaves):
    Device.pin_factory = MockFactory(pin_class=MockPWMPin)
    with UltrasonicTheremin(27, 17, 21, octaves=3) as mock_theremin:
        tone1 = mock_theremin.distance_to_tone(distance_value)
        tone2 = mock_theremin.distance_to_tone(distance_value + 0.01 if distance_value + 0.01 <= 1.0 else distance_value - 0.01)

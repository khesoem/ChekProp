...
@given(
    ...
    controller_interval=st.floats(min_value=0.1, max_value=5),
    mock_interval=st.floats(min_value=0.1, max_value=5)
)
...
def test_cylinder_a_stays_still_when_b_at_bottom(total_time, cylinder_interval, controller_interval, mock_interval):
    ...
    for state in collected_states:
        # When Cylinder B is at the bottom (location 0), Cylinder A's motion should be 0
        if state.cylinder_b_location == 0:

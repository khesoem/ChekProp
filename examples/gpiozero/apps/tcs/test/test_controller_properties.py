...
@given(
    total_time=st.integers(min_value=5, max_value=100),
...
def test_controller_maintains_temp_in_range(total_time, sensor_interval, control_interval, initial_temp):
    ...
    for state in execution_states:
        assert 21 <= state.temp <= 23, f"Temperature out of range: {state.temp}"
...
@given(
    ...
    sensor_interval=st.floats(min_value=0.01, max_value=1.0),
    control_interval=st.floats(min_value=0.01, max_value=1.0),
    initial_temp=st.integers(min_value=24, max_value=30),
)
...
@given(
    ...
    sensor_interval=st.floats(min_value=0.01, max_value=1.0),
    control_interval=st.floats(min_value=0.01, max_value=1.0),
    initial_temp=st.integers(min_value=15, max_value=20),
)
